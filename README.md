...
- OpenRouter free API models ဖြင့် script rewrite လုပ်နိုင်သော Script page
...
- Supported multilingual scripts အတွက် direct Hugging Face VoxCPM2 provider နှင့် Local VoxCPM2 provider
...
- Voice providers: VoxCPM2 Hugging Face Space, local/self-hosted VoxCPM2 API
- Script rewrite provider: OpenRouter API

## ကြိုတင် လိုအပ်ချက်များ

- Node.js `22.12.0` သို့မဟုတ် ပိုသစ်သော version
- npm
- VoxCPM2 သို့မဟုတ် OpenRouter rewrite ကို အသုံးပြုမည်ဆိုပါက Internet connection
...
Default တန်ဖိုးများကို ပြောင်းလိုပါက သို့မဟုတ် OpenRouter rewrite ကို အသုံးပြုလိုပါက `.env.example` ကို `.env.local` အဖြစ် copy လုပ်ပါ။
...
OPENROUTER_REQUEST_TIMEOUT=60000
...
- `VOXCPM_LOCAL_API_URL`: Local သို့မဟုတ် self-hosted VoxCPM2 API ၏ base URL
- `OPENROUTER_REQUEST_TIMEOUT`: OpenRouter request timeout milliseconds
- `OPENROUTER_API_KEY`: Script page မှ rewrite လုပ်ရာတွင် အသုံးပြုသော OpenRouter API key

Script page ရှိ settings dialog မှတစ်ဆင့် `OPENROUTER_API_KEY` ကို `.env.local` ထဲသို့ သိမ်းဆည်းနိုင်သည်။

## Browser တွင် Run ခြင်း

## Local VoxCPM2 Backend

Local provider uses the FastAPI backend in `local-voxcpm2-api/`.
...
Or, after activating the virtualenv, run from the project root:

```powershell
npm run voxcpm2:local
```

The backend exposes `GET /info` and `POST /generate` at `http://localhost:8000`.
The first request may download and load `openbmb/VoxCPM2`, so it can take a while.
CPU mode is very slow; NVIDIA GPU/CUDA is recommended.
...
ရှိပြီးသား script ကို OpenRouter free models ဖြင့် rewrite လုပ်ရန် ဤ page ကို အသုံးပြုပါ။

1. Script tab ကို ဖွင့်ပါ။
2. Original script ကို paste လုပ်ပါ။
3. လိုအပ်ပါက title ထည့်ပါ။
4. OpenRouter model ကို ရွေးပါ။
...
ရွေးချယ်ထားသော model ကို configured OpenRouter API endpoint က လက်ခံနိုင်ရမည်။
...
| `Local VoxCPM2` | `VOXCPM_LOCAL_API_URL` တွင် run နေသော local/self-hosted VoxCPM2 API ကို အသုံးပြုသည် |
...
| Provider | `voxcpm2`, `voxcpm2_local` သို့မဟုတ် `burmese_production` |
...
| Reference audio | VoxCPM2, Local VoxCPM2 နှင့် Burmese Production အတွက် required |
...
VoxCPM2 request များအတွက် script ကို အများဆုံး `220` characters ပါသော chunks များအဖြစ် ခွဲသည်။ Public VoxCPM2 Space သို့မဟုတ် Local VoxCPM2 API က compressed MP3 chunk ပြန်ပေးလာသည်နှင့် Thalika သည် disk ပေါ်သို့ MP3 file မရေးမီ memory ထဲတွင် ချက်ချင်း decode လုပ်သည်။ ထို့နောက် real `48kHz` mono `24-bit PCM WAV` temporary chunk အဖြစ် validate လုပ်ပြီးမှ သိမ်းသည်။ Pipeline ၏ နောက်ဆက်တွဲအဆင့်များတွင် MP3 ကို မသုံးတော့ပါ။
...
| `GET` | `/api/providers/voxcpm2-local/health` | Local/self-hosted VoxCPM2 API ကို probe လုပ်ခြင်း |
| `POST` | `/api/rewrite` | OpenRouter ဖြင့် script rewrite လုပ်ခြင်း |
| `GET`, `POST` | `/api/settings/openrouter` | OpenRouter API key state ဖတ်ခြင်း သို့မဟုတ် သိမ်းဆည်းခြင်း |
...
Voice Over page သည် selected provider အပေါ်မူတည်ပြီး `/api/providers/voxcpm2/health` သို့မဟုတ် `/api/providers/voxcpm2-local/health` ကို ခေါ်သည်။
...
- VoxCPM2 health badge
- `HF_VOXCPM2_URL`
- `VOXCPM_LOCAL_API_URL`
- `HF_REQUEST_TIMEOUT`
- `HF_INFERENCE_TIMEOUT`
- `data/logs/generation.log`

OpenRouter rewrite အလုပ်မလုပ်ပါက အောက်ပါ အချက်များကို စစ်ပါ။

- `OPENROUTER_API_KEY`
- `OPENROUTER_REQUEST_TIMEOUT`
- Configured OpenRouter API က ရွေးချယ်ထားသော model ကို လက်ခံခြင်း ရှိ၊ မရှိ
